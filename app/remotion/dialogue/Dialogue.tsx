			...
			<div className="absolute inset-0 bg-gradient-to-br from-indigo-100 via-blue-50 to-purple-100">
				...
				<div className="absolute inset-0 flex items-center justify-center">
					{dialogues.map((dialogue, index) => {
						...
						const slideY = spring({ fps, frame: frame - startFrame, from: 30, to: 0, config: { damping: 12 } })
						...
						// Random offset for natural feel (minimal to keep consistent positioning)
						const randomOffset = (random(index) * 2 - 1) * 5

						return (
							<Sequence key={`${dialogue.roleLabel}-${dialogue.content}-${index}`} from={startFrame} durationInFrames={DIALOGUE_DURATION}>
								{/* Main content container - keeps everything in the middle area of screen */}
								<div className="absolute inset-0 flex items-center justify-center">
									<div className="relative w-[90%] h-[70%] flex items-center">
										{/* Card positioning wrapper */}
										<div
											className={`
												absolute
												${isRole1 ? 'left-0 top-[30%]' : 'right-0 bottom-[30%]'}
												w-[85%]
												mx-auto
											`}
										>
											...
											<div
												...
												style={{
													opacity,
													transform: `scale(${scale}) translateY(${slideY * (isRole1 ? -1 : 1)}px) translateX(${randomOffset}px)`,
											...
											>
												...
												<div
													className={`
														absolute 
														${isRole1 ? 'top-0 left-0 right-0 h-3' : 'bottom-0 left-0 right-0 h-3'}
												...
												<div className="p-10 pt-12">
													...
													<div className={`flex ${isRole1 ? 'flex-row' : 'flex-row-reverse'} items-start gap-8`}>
														{/* Avatar */}
														<div
