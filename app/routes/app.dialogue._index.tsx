			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
					...
					<CardContent className="p-6">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-3xl font-bold text-foreground">{dialogues.length}</p>
							</div>
				...
				<Card className="border-border/50 shadow-soft bg-card/50">
					<CardContent className="p-6">
						<div className="flex items-center justify-between">
							<div>
								<p className="text-sm font-medium text-muted-foreground">Total Dialogues</p>
								<p className="text-3xl font-bold text-foreground">{totalDialogues}</p>
							</div>
							<div className="h-12 w-12 rounded-lg bg-primary/10 flex items-center justify-center">
			...
			{dialogues.length === 0 ? (
				<Card className="border-border/50 shadow-soft">
					<CardContent className="p-12">
						<div className="text-center">
							...
							<h3 className="text-xl font-semibold text-foreground mb-2">No dialogues yet</h3>
							<p className="text-muted-foreground mb-6 max-w-md mx-auto">Create your first dialogue exercise. Build engaging conversations with AI assistance.</p>
							<Link to="/app/dialogue/new">
								<Button className="shadow-soft">
									<Plus className="h-4 w-4 mr-2" />
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
					{dialogues.map((dialogue) => (
						<Card key={dialogue.id} className="group border-border/50 shadow-soft hover:shadow-medium transition-all duration-300 hover:-translate-y-1 bg-card/50 backdrop-blur-sm">
							<CardHeader className="pb-3">
								...
								<CardTitle className="text-lg font-semibold text-foreground line-clamp-2 leading-tight">Dialogue {dialogue.id.slice(0, 8)}</CardTitle>
								...
								<div className="space-y-4">
									...
									<div className="grid grid-cols-2 gap-3">
										<div className="p-3 bg-muted/50 rounded-lg">
											...
											<p className="text-sm font-medium text-foreground">{dialogue.dialogues?.length || 0}</p>
										</div>
										<div className="p-3 bg-muted/50 rounded-lg">
											<p className="text-xs text-muted-foreground mb-1">Frame Rate</p>
											<p className="text-sm font-medium text-foreground">{dialogue.fps} FPS</p>
										</div>
									</div>
									...
									{dialogue.jobId && (
										<div className="p-3 bg-muted/50 rounded-lg">
											...
											<p className="text-sm text-foreground truncate font-mono">{dialogue.jobId}</p>
										</div>
									)}
