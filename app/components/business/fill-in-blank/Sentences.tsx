...
import { Clock, Headphones, Trash2, Upload, Volume2 } from 'lucide-react'
...
export default function Sentences({ sentences }: SentencesProps) {
	...
	// Check if the current fetcher operation is for a specific index
	const isGeneratingAudio = (index: number) => {
		const formData = generateAudioFetcher.formData
		if (!formData) return false
		const fetcherIndex = formData.get('index')
		return generateAudioFetcher.state === 'submitting' && fetcherIndex === index.toString()
	}

	return (
		<div className="space-y-6">
			{sentences.map((sentence, index) => {
				const hasLongDuration = sentence.audioDuration && sentence.audioDuration > 10
						...
						<div className="space-y-4">
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									<div className="flex items-center gap-2">
										...
										{hasLongDuration && (
											<Badge className="flex items-center gap-1 bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100">
												<Clock className="h-3 w-3" />
												<span className="text-xs">Long Audio</span>
											</Badge>
										)}
									</div>
								</div>
							...
							<div className="flex flex-col gap-2">
								<div className="flex items-center gap-3 rounded-xl bg-gradient-to-r from-blue-50 to-purple-50 p-4">
									...
									<generateAudioFetcher.Form method="post" action="generate-single-audio">
										<input type="hidden" name="index" value={index} />
										<LoadingButtonWithState
											state={isGeneratingAudio(index) ? 'submitting' : 'idle'}
											idleText=""
											className="h-9 w-9 p-0 rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 text-white hover:from-blue-600 hover:to-indigo-600"
										>
							...
							{(sentence.audioFilePath || sentence.publicAudioPath) && (
								<div className="mt-2">
									...
									{sentence.audioDuration && (
										<p className={`text-xs ${hasLongDuration ? 'text-amber-600 font-medium' : 'text-gray-500'} mt-1`}>
											Duration: {sentence.audioDuration.toFixed(2)}s{hasLongDuration && ' (Recommended: <10s)'}
										</p>
									)}
								</div>
							)}
